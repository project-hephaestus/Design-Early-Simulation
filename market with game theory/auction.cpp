...
//in order to take the demand for each town 
int counter1=1;
int counter2=1;
int counter3=1;
int counter4=1;
char *iname1 = "demand1.txt";
char *iname2 = "demand2.txt";
char *iname3 = "demand3.txt";
char *iname4 = "demand4.txt";
int market1[24];
int market2[24];
int market3[24];
int market4[24];		
...
/**/		PT_object, "network", PADDR(network), PT_DESCRIPTION, "the comm network used by object to talk to the market (if any)",
			...
			PT_object, "town_factory", PADDR(town_factory),PT_DESCRIPTION, "the factory of each town",
			PT_object, "second_town_factory", PADDR(second_town_factory),PT_DESCRIPTION, "the second factory of each town which opens when the demand is not satisfied from the first town_factory",
			PT_object, "second_bidder", PADDR(second_bidder),PT_DESCRIPTION, "the bidder which bids at second level auction",
			PT_int64, "type", PADDR(type),PT_DESCRIPTION, "0 for auction town 1 for second level",
...
int auction::create(void)
{
	...
	/*read the demand files*/
	ifstream infile(iname1);
	ifstream infile2(iname2);
	ifstream infile3(iname3);
	ifstream infile4(iname4);
...
int auction::init(OBJECT *parent)
{
	...
	unsigned int i = 0;
	//second_town_factory=NULL;
	if(second_bidder==NULL)
	{
	type=1; //the auction is second level
	}
	if(trans_log[0] != 0){
...
void auction::clear_market(void)
{
  ...
  if(type==0)
  {
	    OBJECT *bidder=gl_get_object(town_factory->name);
		OBJECT *sec_bidder=gl_get_object(second_bidder->name);
		

		double max_capacity=0.0;
		int percentage;
		double d_0;//the initial quantity for the game theory case
		
		

		double *seller_price=gl_get_double_by_name(bidder,"price");
		double *cost=gl_get_double_by_name(bidder,"cost");
		...
		sprintf(s5,"%f",*seller_price);
		sprintf(s2,"%f",1.2*(*seller_price));
		sprintf(s6,"%f",asks.get_max());
		sprintf(s7,"%f",current_frame.buyer_total_quantity);
		sprintf(s1,"%f",offers.get_max());
		sprintf(s8,"%f",current_frame.seller_total_quantity);
		
		
		if(strcmp(OBJECTHDR(this)->name,"Market_town1")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
			 dd++;
		}
		else if(strcmp(OBJECTHDR(this)->name,"Market_town2")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town3")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town4")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town5")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town6")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town7")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town8")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town9")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_town10")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_volos")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_karditsa")!=1)
		{
			// sprintf(s9,"%d",5591-(5591*percentage)/100);
			// counter1++;
			printf("mia gora");
	...
	/*	else if(strcmp(OBJECTHDR(this)->name,"Market_2")!=1)
		{
			   if(counter2<12)
				{
					sprintf(s9,"%d",1585-(1585*percentage)/100);
				}
				else
				{
		...
		else if(strcmp(OBJECTHDR(this)->name,"Market_3")!=1)
		{
			    
			     sprintf(s9,"%d",3697-(3697*percentage)/100);
				 counter3++;
				 dd++;
		}
		else if(strcmp(OBJECTHDR(this)->name,"Market_4")!=1)
		{
			  
			 	if(counter4<12)
				{	
					sprintf(s9,"%d",12131+(12131*percentage)/100);
				}
				else
				{
				  sprintf(s9,"%d",12131-(12131*percentage)/100);
		...
		if(current_frame.buyer_total_quantity > current_frame.seller_total_quantity)
		{
			d_0=current_frame.seller_total_quantity;
		}
		else
		{
			d_0=current_frame.buyer_total_quantity;
		}
		
		*/
		
	    
		gl_set_value_by_name(bidder,"quantity",s9);
		//printf("name:%s supply: %s demand :%s clearing %f \n",OBJECTHDR(this)->parent->name,s9,s7,current_frame.clearing_quantity);
		/*condition A*/
		if(current_frame.seller_total_quantity >=current_frame.buyer_total_quantity )
		{
			
			sprintf(s3,"%f",current_frame.seller_total_quantity-current_frame.clearing_quantity);
			printf("supply:%s for the town factory at price:%s  \n",s3,s5);
			gl_set_value_by_name(sec_bidder,"price",s5);	
			gl_set_value_by_name(sec_bidder,"quantity",s3);
			gl_set_value_by_name(sec_bidder,"role","SELLER"); //seller
			//gl_set_value_by_name(bidder,"change","0");

		}
		
		
		else if(current_frame.clearing_quantity < current_frame.buyer_total_quantity)
		{
			  
				sprintf(s4,"%f",current_frame.buyer_total_quantity-current_frame.clearing_quantity);
				//the town factory bids now at second level auction all its quantity 
				/*condition B*/
				if(second_town_factory==NULL)
				{
					printf("EDW MPANITI");
					gl_set_value_by_name(sec_bidder,"quantity",s4);
					gl_set_value_by_name(sec_bidder,"price",s6);
					gl_set_value_by_name(sec_bidder,"role","BUYER"); //buyer
				}
				else
				{    /*condition C*/
					OBJECT *second_factory=gl_get_object(second_town_factory->name);
					printf("SECOND LEVEL\n");
					gl_set_value_by_name(second_factory,"quantity",s4);
				    //gl_set_value_by_name(second_factory,"change","2");
					gl_set_value_by_name(second_factory,"price",s2);
					gl_set_value_by_name(second_factory,"role","SELLER"); //seller
	...
	/****FIND GAME THEORY SOLUTION*****/
 
	
    	
  	 
   /****END FIND GAME THEORY SOLUTION*****/
